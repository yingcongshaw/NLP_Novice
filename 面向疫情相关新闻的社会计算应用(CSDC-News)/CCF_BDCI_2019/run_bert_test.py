...
def convert_examples_to_features(examples, tokenizer, max_seq_length, split_num, is_training):
    ...
    for example_index, example in enumerate(examples):
        ...
        for i in range(split_num):
            context_tokens_choice = context_tokens[ int(i*skip_len) : int((i+1)*skip_len)]   #split_num：将文本分割成split_num段
...
def main():
        ...
        for step in bar:
            ...
            if args.do_eval and (step + 1) %(args.eval_steps*args.gradient_accumulation_steps)==0:
                ...
                for file in ['dev.csv']:
                    ...
                    eval_accuracy = computeF1(inference_logits, gold_labels)
    ...
    if args.do_test:
        ...
        for file,flag in [('test.csv','test')]:
            ...
            for x,(input_ids, input_mask, segment_ids, label_ids) in enumerate(eval_dataloader):
                input_ids = input_ids.to(device)
                input_mask = input_mask.to(device)
                segment_ids = segment_ids.to(device)
                label_ids = label_ids.to(device)
                ...
                # if (x+1) % 200 == 0:
                #     print('epoch:', (x+1))
                #     l=np.concatenate(inference_labels,0)
                #     df = pd.read_csv(os.path.join(args.data_dir, file))
                #     df['label_0'] = l[:,0]
                #     df['label_1'] = l[:,1]
                #     df['label_2'] = l[:,2]
                #     df[['id','label_0','label_1','label_2']].to_csv(os.path.join(args.output_dir, "predicted_test_{}.csv".format(str(x))), index=False)
                print('epoch:', (x+1))

            gold_labels=np.concatenate(gold_labels,0)
            logits=np.concatenate(inference_labels,0)

            print(flag, computeF1(logits, gold_labels))
            ...
            if flag == 'dev':
                ...
                df['label_1'] = logits[:,1]
                df['label_2'] = logits[:,2]
                df[['id','label_0','label_1','label_2']].to_csv(os.path.join(args.output_dir, "predicted_dev.csv"), index=False)
            ...
            if flag == 'test':
                ...
                df['label_1'] = logits[:,1]
                df['label_2'] = logits[:,2]
                df[['id','label_0','label_1','label_2']].to_csv(os.path.join(args.output_dir, "predicted_test.csv"), index=False)
